...
exports.sendTrackingEmailTemplate = exports.generateLowStockAlertEmailTemplate = exports.generateTransferEmailTemplate = exports.generateOrderEmailTemplate = void 0;
const mjml_1 = __importDefault(require("mjml"));
// Template para orden general (pago con tarjeta, etc.)
const generateOrderEmailTemplate = (order) => {
    var _a, _b;
    const itemsRows = order.items
      ...
      `)
        .join('');
    const transferHtml = ((_a = order.transferDiscount) === null || _a === void 0 ? void 0 : _a.applied)
        ? `
    <tr>
      ...
      <td align="right">- $${order.transferDiscount.value.toFixed(2)}</td>
        ...
        : '';
    const couponHtml = ((_b = order.couponDiscount) === null || _b === void 0 ? void 0 : _b.applied)
        ? `
    <tr>
...
<mjml>
  ...
  `;
    return (0, mjml_1.default)(mjml).html;
};
exports.generateOrderEmailTemplate = generateOrderEmailTemplate;
// Template para orden con transferencia
const generateTransferEmailTemplate = (order) => {
    const itemsRows = order.items
        .map(item => `
      <tr style="border-bottom:1px solid #ccc;">
        <td>${item.quantity} x ${item.product.name}</td>
...
exports.generateTransferEmailTemplate = generateTransferEmailTemplate;
const generateLowStockAlertEmailTemplate = (name, sku, id, stock) => {
    return `
<mjml>
  <mj-head>
...
exports.generateLowStockAlertEmailTemplate = generateLowStockAlertEmailTemplate;
const sendTrackingEmailTemplate = (order) => {
    const itemsRows = order.items
        .map(item => `
...
exports.sendTrackingEmailTemplate = sendTrackingEmailTemplate;
